...
def obtener_asistencias(
    ...
    _ = Depends(RoleChecker(["superadmin", "admin", "rrhh"])) # Necesario para que el módulo de Reportes funcione
):
...
def marcar_salida(
    ...
    asistencia_actualizada = crud.mark_salida(db=db, asistencia_id=asistencia_id)
...
def editar_asistencia_admin(
    ...
    asistencia_actualizada = crud.update_asistencia_manual(db, asistencia_id, asistencia_in.model_dump(exclude_unset=True))
    if not asistencia_actualizada:
        raise HTTPException(status_code=404, detail="Asistencia no encontrada")
    return asistencia_actualizada